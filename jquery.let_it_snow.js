  ...
  $.fn.let_it_snow = function(options){
        ...
        for (var i = 0; i < flakeCount; i++) {
            ...
            if (settings.image == false) {
              ...
              if(typeof settings.image === "string") {
                ...
                ctx.drawImage($("img.lis_flake"+imageSeries[i]).get(0), flake.x, flake.y, flake.size * 2, flake.size * 2);
    ...
    function reset(flake) {
     ...
     function init() {
      ...
      if(typeof settings.image !== 'string') {
        for(var i=0; i < flakeCount; i++){
          imageSeries[i] = Math.ceil(Math.random()*settings.image.length) - 1;
        }
      }
      snow();
    }
    ...
    if (settings.image != false) {
      if( typeof settings.image === 'string') {
        ...
        settings.image.forEach(function(image, index) {
          $("<img src='"+image+"' style='display: none' class='lis_flake"+index+"'>").prependTo("body")
        });
      }
    }
